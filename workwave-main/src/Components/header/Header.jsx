...
export default function Header(props) {
            ...
            if(localStorage.getItem("email")) {
                try {
                    const response = axios.get(`${baseURL}/api/notification/all`, {
                        ...
                        setCont(res.data.data.filter(notification => !notification.checked).length)
                    })
                  } catch(err) {
                    console.log(err);
                  }
    ...
    const checkNotification = async (id) => {
        try {
            const response = axios.put(`${baseURL}/api/notification/${id}`, {}, {
                withCredentials: true,
            }).then((res) => {
                setNotification(prevNotifications => 
                    prevNotifications.map(notification =>
                        notification._id === id ? { ...notification, checked: true } : notification
                    )
                );
                setCont(notification.filter(notification => !notification.checked).length)
                    ...
                    <Logo className={isPhone ? 'hidden' : ''} />
                        <>
                            ...
                            }
                            {
                                ...
                                <div className="nav">
                                    <NavLink to={"/admin/users/freelancers"} className='link admin'>Users</NavLink>
                                    <NavLink to={"/admin/banned"} className='link admin'>Blocked Users</NavLink>
                                    <NavLink to={"/admin/reports"} className='link admin'>Reports</NavLink>
                                </div> 
                            :
                            ""
                            }
                                ...
                                {!hasAccount ?
                                    ...
                                    <>
                                        ...
                                        <div className='relative'>
                                            <span className='absolute right-0 top-0 w-5 h-5 bg-red-700 text-white rounded-full text-xs flex justify-center items-center z-30'>{cont}</span>
                                            <button onClick={() => {
                                                ...
                                                <div className=" bg-gray-50 h-96 overflow-y-auto pt-8 px-4 right-0">
                                                    ...
                                                    </div>
                                                    {
                                                        notification?.map((item, index) => 
                                                            <div key={index} onClick={() => {checkNotification(item._id)}} className={`w-full p-3 mt-4 ${item.checked ? "bg-white": "bg-[#00afef53]"} rounded shadow flex flex-shrink-0`}>
                                        ...
                                        <div onClick={() => {
                                            ...
                                            <div className={"relative border-b-4 border-transparent border-indigo-700 transform transition duration-300"}  x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100">
                                                ...
                                                <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-[#00AEEF]">
                                                    <img src={localStorage.getItem("photo") == "null" ? "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png" : localStorage.getItem("photo")} alt="" className="w-full h-full object-cover" />
                                                </div>
                                                <div className="text-gray-900 select-none">
                                                    ...
                                                    <div className="cursor-pointer text-xs font-light">{localStorage.getItem("role") == "freelencer" ? "FREELANCER" : localStorage.getItem("role") == "user" ?"CLIENT" : "ADMIN"}</div>
                                                </div>
                                                </div>
                                                ...
                                                <ul className="space-y-3 ">
                                                  ...
                                                  <>
                                                    <li className="font-medium">
                                                        ...
                                                        <li className="font-medium">
                                                        <Link to={localStorage.getItem("role") == "freelencer" ? "/work/personalspace/dashboard" : "/hire/personalspace/dashboard"} className="flex items-center transform transition-colors duration-200 border-r-4 border-transparent hover:border-[#00AEEF]">
                                                            <div className="mr-3">
                                                        ...
                                                        <li className="font-medium">
                                                        <Link to={localStorage.getItem("role") == "freelencer" ? "/work/addService" : "/hire/addneed"} className="flex items-center transform transition-colors duration-200 border-r-4 border-transparent hover:border-[#00AEEF]">
                                                            <div className="mr-3">
                                                            <FontAwesomeIcon icon={faAdd} className='text-xl ml-1 order-1 cursor-pointer' />
                                                            </div>
                                                            {localStorage.getItem("role") == "freelencer" ? "Add a Service" : "Add a need"}
                                                        </Link>
                                                    </li>
                                                  </> : <></>
                                                  }
