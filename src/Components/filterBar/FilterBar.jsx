...
const FilterBar = () => {
    ...
    return(
        ...
        <form id="FilterBar" className={`max-sm:hidden flex justify-center mt-16 mx-20 drop-shadow-xl bg-white rounded-br-3xl fixed top-0 rounded-bl-3xl p-4 max-md:w-[80%] w-[65%] z-10 ${isFixed ? "" : "opacity-0 hover:opacity-100 transition-all duration-300"}`}>
                ...
                />
                <div id="clientNeeds" className="max-md:text-xs p-10 flex gap-3 justify-center text-sm w-full">
                    ...
                    <Link className={'btn block w-1/5 flex justify-around max-sm:flex-col" items-center'} >
                        <span className='max-md:text-sm text-lg font-extrabold primaryfont block'>Filter</span>
                        <FontAwesomeIcon icon={faSliders} className="max-md:h-3 ml-1 h-5"/>
                    </Link>
                </div>
            </form>
    )
