...
import { CERTIFICATE, SENDCERTIFICATE, baseURL, greyColor } from '../../../../../Components/Variables/Variables';
        ...
        try {
            const response = await axios.post(`${baseURL}/${CERTIFICATE}`, formData, {
                ...
                setSkillsAndCertificate((prevState) => ({
                    ...prevState,
                    certificate: [...prevState.certificate, res.data[0]], // Updating the skills property with new data
                }));
        ...
        if (skillsAndCertificate.skills.length != 0) {
            try {
                const sendEmail = await axios.post(`${baseURL}/${SENDCERTIFICATE}`, skillsAndCertificate, {
                    withCredentials: true,
                }).then(res => { navigate("profiledetails"), console.log(skillsAndCertificate) });
            } catch (err) {
